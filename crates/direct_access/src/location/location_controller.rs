...
use super::{
    dtos::{CreateLocationDto, LocationDto, UpdateLocationDto},
    units_of_work::{LocationReadUoWFactory, LocationWriteUoWFactory},
};
...
pub fn create_orphan(
    ...
    stack_id: Option<u64>,
    entity: &CreateLocationDto,
) -> Result<LocationDto> {
    let uow_factory = LocationWriteUoWFactory::new(db_context, event_hub);
    let mut uc = use_cases::UndoableCreateOrphanUseCase::new(uow_factory);
    let entity_in: common::entities::Location = entity.into();
...
pub fn create_orphan_multi(
    ...
    undo_redo_manager: &mut UndoRedoManager,
    stack_id: Option<u64>,
    entities: &[CreateLocationDto],
) -> Result<Vec<LocationDto>> {
    let uow_factory = LocationWriteUoWFactory::new(db_context, event_hub);
    let entities_in: Vec<common::entities::Location> =
...
pub fn get(db_context: &DbContext, id: &EntityId) -> Result<Option<LocationDto>> {
    let uow_factory = LocationReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute(id)?.map(|e| e.into()))
}

pub fn get_all(db_context: &DbContext) -> Result<Vec<LocationDto>> {
    let uow_factory = LocationReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc.execute_all()?.into_iter().map(|e| e.into()).collect())
}

pub fn get_multi(db_context: &DbContext, ids: &[EntityId]) -> Result<Vec<Option<LocationDto>>> {
    let uow_factory = LocationReadUoWFactory::new(db_context);
    let uc = use_cases::GetUseCase::new(uow_factory);
    Ok(uc
        .execute_multi(ids)?

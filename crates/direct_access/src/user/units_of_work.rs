...
impl CommandUnitOfWork for UserWriteUoW {
...
impl use_cases::WriteUoW for UserWriteUoW {
    type Entity = User;

    fn get(&self, id: &EntityId) -> Result<Option<User>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_user_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<User>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_user_repository(transaction)?;
    ...
    fn create_orphan_multi(&self, entities: &[User]) -> Result<Vec<User>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_user_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[User]) -> Result<Vec<User>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_user_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[User]) -> Result<Vec<User>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_user_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for UserWriteUoW {
    fn create_multi(&self, entities: &[User], owner_id: EntityId, index: i32) -> Result<Vec<User>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_user_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::WriteRelUoW<UserRelationshipField> for UserWriteUoW {
    fn set_relationship(
        &self,
        id: &EntityId,
...
pub struct UserWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl UserWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        UserWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
...
impl use_cases::WriteUoWFactory for UserWriteUoWFactory {
    type Entity = User;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = User>> {
        Box::new(UserWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for UserWriteUoWFactory {
    type Entity = User;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = User>> {
        Box::new(UserWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<UserRelationshipField> for UserWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<UserRelationshipField>> {
        Box::new(UserWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for UserReadUoW {
...
impl use_cases::ReadUoW for UserReadUoW {
    type Entity = User;

    fn get(&self, id: &EntityId) -> Result<Option<User>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_user_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<UserRelationshipField> for UserReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &UserRelationshipField,
...
pub struct UserReadUoWFactory {
    context: DbContext,
}

impl UserReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        UserReadUoWFactory {
            context: db_context.clone(),
        }
...
impl use_cases::ReadUoWFactory for UserReadUoWFactory {
    type Entity = User;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = User>> {
        Box::new(UserReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<UserRelationshipField> for UserReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<UserRelationshipField>> {
        Box::new(UserReadUoW::new(&self.context))
    }
}

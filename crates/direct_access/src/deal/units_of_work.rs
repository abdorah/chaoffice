...
impl CommandUnitOfWork for DealWriteUoW {
...
impl use_cases::WriteUoW for DealWriteUoW {
    type Entity = Deal;

    fn get(&self, id: &EntityId) -> Result<Option<Deal>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_deal_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Deal>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_deal_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<Deal>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_deal_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[Deal]) -> Result<Vec<Deal>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_deal_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Deal]) -> Result<Vec<Deal>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_deal_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Deal]) -> Result<Vec<Deal>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_deal_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for DealWriteUoW {
    fn create_multi(&self, entities: &[Deal], owner_id: EntityId, index: i32) -> Result<Vec<Deal>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_deal_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::WriteRelUoW<DealRelationshipField> for DealWriteUoW {
    fn set_relationship(
        &self,
        id: &EntityId,
...
pub struct DealWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl DealWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        DealWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
...
impl use_cases::WriteUoWFactory for DealWriteUoWFactory {
    type Entity = Deal;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Deal>> {
        Box::new(DealWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for DealWriteUoWFactory {
    type Entity = Deal;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Deal>> {
        Box::new(DealWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<DealRelationshipField> for DealWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<DealRelationshipField>> {
        Box::new(DealWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for DealReadUoW {
...
impl use_cases::ReadUoW for DealReadUoW {
    type Entity = Deal;

    fn get(&self, id: &EntityId) -> Result<Option<Deal>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_deal_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<DealRelationshipField> for DealReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &DealRelationshipField,
...
pub struct DealReadUoWFactory {
    context: DbContext,
}

impl DealReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        DealReadUoWFactory {
            context: db_context.clone(),
        }
...
impl use_cases::ReadUoWFactory for DealReadUoWFactory {
    type Entity = Deal;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Deal>> {
        Box::new(DealReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<DealRelationshipField> for DealReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<DealRelationshipField>> {
        Box::new(DealReadUoW::new(&self.context))
    }
}

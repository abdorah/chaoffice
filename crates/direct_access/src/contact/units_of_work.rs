...
impl CommandUnitOfWork for ContactWriteUoW {
...
impl use_cases::WriteUoW for ContactWriteUoW {
    type Entity = Contact;

    fn get(&self, id: &EntityId) -> Result<Option<Contact>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_contact_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Contact>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_contact_repository(transaction)?;
    ...
    fn create_orphan_multi(&self, entities: &[Contact]) -> Result<Vec<Contact>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_contact_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Contact]) -> Result<Vec<Contact>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_contact_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Contact]) -> Result<Vec<Contact>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_contact_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for ContactWriteUoW {
    fn create_multi(
        &self,
        entities: &[Contact],
        owner_id: EntityId,
...
pub struct ContactWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl ContactWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        ContactWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for ContactWriteUoWFactory {
    type Entity = Contact;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Contact>> {
        Box::new(ContactWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for ContactWriteUoWFactory {
    type Entity = Contact;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Contact>> {
        Box::new(ContactWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for ContactReadUoW {
...
impl use_cases::ReadUoW for ContactReadUoW {
    type Entity = Contact;

    fn get(&self, id: &EntityId) -> Result<Option<Contact>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_contact_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
pub struct ContactReadUoWFactory {
    context: DbContext,
}

impl ContactReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        ContactReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for ContactReadUoWFactory {
    type Entity = Contact;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Contact>> {
        Box::new(ContactReadUoW::new(&self.context))
    }
}

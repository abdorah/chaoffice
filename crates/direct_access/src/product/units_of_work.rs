...
impl CommandUnitOfWork for ProductWriteUoW {
...
impl use_cases::WriteUoW for ProductWriteUoW {
    type Entity = Product;

    fn get(&self, id: &EntityId) -> Result<Option<Product>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_product_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Product>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_product_repository(transaction)?;
    ...
    fn create_orphan_multi(&self, entities: &[Product]) -> Result<Vec<Product>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_product_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Product]) -> Result<Vec<Product>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_product_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Product]) -> Result<Vec<Product>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_product_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for ProductWriteUoW {
    fn create_multi(
        &self,
        entities: &[Product],
        owner_id: EntityId,
...
impl use_cases::WriteRelUoW<ProductRelationshipField> for ProductWriteUoW {
    fn set_relationship(
        &self,
        id: &EntityId,
...
pub struct ProductWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl ProductWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        ProductWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for ProductWriteUoWFactory {
    type Entity = Product;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Product>> {
        Box::new(ProductWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for ProductWriteUoWFactory {
    type Entity = Product;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Product>> {
        Box::new(ProductWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<ProductRelationshipField> for ProductWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<ProductRelationshipField>> {
        Box::new(ProductWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for ProductReadUoW {
...
impl use_cases::ReadUoW for ProductReadUoW {
    type Entity = Product;

    fn get(&self, id: &EntityId) -> Result<Option<Product>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_product_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<ProductRelationshipField> for ProductReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &ProductRelationshipField,
...
pub struct ProductReadUoWFactory {
    context: DbContext,
}

impl ProductReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        ProductReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for ProductReadUoWFactory {
    type Entity = Product;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Product>> {
        Box::new(ProductReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<ProductRelationshipField> for ProductReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<ProductRelationshipField>> {
        Box::new(ProductReadUoW::new(&self.context))
    }
}

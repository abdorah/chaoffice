...
impl CommandUnitOfWork for PersonWriteUoW {
...
impl use_cases::WriteUoW for PersonWriteUoW {
    type Entity = Person;

    fn get(&self, id: &EntityId) -> Result<Option<Person>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_person_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Person>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_person_repository(transaction)?;
        Ok(repo.get_multi(ids)?)
    }

    fn get_all(&self) -> Result<Vec<Person>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_person_repository(transaction)?;
        Ok(repo.get_all()?)
    }

    fn create_orphan_multi(&self, entities: &[Person]) -> Result<Vec<Person>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_person_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Person]) -> Result<Vec<Person>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_person_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Person]) -> Result<Vec<Person>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_person_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for PersonWriteUoW {
    fn create_multi(
        &self,
        entities: &[Person],
        owner_id: EntityId,
...
impl use_cases::WriteRelUoW<PersonRelationshipField> for PersonWriteUoW {
    fn set_relationship(
        &self,
        id: &EntityId,
...
pub struct PersonWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl PersonWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        PersonWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for PersonWriteUoWFactory {
    type Entity = Person;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Person>> {
        Box::new(PersonWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for PersonWriteUoWFactory {
    type Entity = Person;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Person>> {
        Box::new(PersonWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<PersonRelationshipField> for PersonWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<PersonRelationshipField>> {
        Box::new(PersonWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
impl QueryUnitOfWork for PersonReadUoW {
...
impl use_cases::ReadUoW for PersonReadUoW {
    type Entity = Person;

    fn get(&self, id: &EntityId) -> Result<Option<Person>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_person_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<PersonRelationshipField> for PersonReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &PersonRelationshipField,
...
pub struct PersonReadUoWFactory {
    context: DbContext,
}

impl PersonReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        PersonReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for PersonReadUoWFactory {
    type Entity = Person;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Person>> {
        Box::new(PersonReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<PersonRelationshipField> for PersonReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<PersonRelationshipField>> {
        Box::new(PersonReadUoW::new(&self.context))
    }
}

...
impl CommandUnitOfWork for CategoryWriteUoW {
...
impl use_cases::WriteUoW for CategoryWriteUoW {
    type Entity = Category;

    fn get(&self, id: &EntityId) -> Result<Option<Category>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_category_repository(transaction)?;
        Ok(repo.get(id)?)
    }

    fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Category>>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let repo = repository_factory::write::create_category_repository(transaction)?;
    ...
    fn create_orphan_multi(&self, entities: &[Category]) -> Result<Vec<Category>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_category_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
        Ok(repo.create_orphan_multi(&mut event_buffer, entities)?)
    }

    fn update_multi(&self, entities: &[Category]) -> Result<Vec<Category>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_category_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
    ...
    fn update_with_relationships_multi(&self, entities: &[Category]) -> Result<Vec<Category>> {
        let transaction = self.transaction.as_ref().expect("Transaction not started");
        let mut repo = repository_factory::write::create_category_repository(transaction)?;
        let mut event_buffer = self.event_buffer.borrow_mut();
...
impl use_cases::OwnedWriteUoW for CategoryWriteUoW {
    fn create_multi(
        &self,
        entities: &[Category],
        owner_id: EntityId,
...
impl use_cases::WriteRelUoW<CategoryRelationshipField> for CategoryWriteUoW {
    fn set_relationship(
        &self,
        id: &EntityId,
...
pub struct CategoryWriteUoWFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl CategoryWriteUoWFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        CategoryWriteUoWFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl use_cases::WriteUoWFactory for CategoryWriteUoWFactory {
    type Entity = Category;
    fn create(&self) -> Box<dyn use_cases::WriteUoW<Entity = Category>> {
        Box::new(CategoryWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::OwnedWriteUoWFactory for CategoryWriteUoWFactory {
    type Entity = Category;
    fn create(&self) -> Box<dyn use_cases::OwnedWriteUoW<Entity = Category>> {
        Box::new(CategoryWriteUoW::new(&self.context, &self.event_hub))
    }
}

impl use_cases::WriteRelUoWFactory<CategoryRelationshipField> for CategoryWriteUoWFactory {
    fn create(&self) -> Box<dyn use_cases::WriteRelUoW<CategoryRelationshipField>> {
        Box::new(CategoryWriteUoW::new(&self.context, &self.event_hub))
    }
}
...
pub struct CategoryReadUoW {
    context: DbContext,
    transaction: RefCell<Option<Transaction>>,
}

impl CategoryReadUoW {
...
impl QueryUnitOfWork for CategoryReadUoW {
...
impl use_cases::ReadUoW for CategoryReadUoW {
    type Entity = Category;

    fn get(&self, id: &EntityId) -> Result<Option<Category>> {
        let transaction = self.transaction.borrow();
        let repo = repository_factory::read::create_category_repository(
            transaction
                .as_ref()
                .ok_or_else(|| anyhow::anyhow!("No active transaction"))?,
        )?;
...
impl use_cases::ReadRelUoW<CategoryRelationshipField> for CategoryReadUoW {
    fn get_relationship(
        &self,
        id: &EntityId,
        field: &CategoryRelationshipField,
...
pub struct CategoryReadUoWFactory {
    context: DbContext,
}

impl CategoryReadUoWFactory {
    pub fn new(db_context: &DbContext) -> Self {
        CategoryReadUoWFactory {
            context: db_context.clone(),
        }
    }
}

impl use_cases::ReadUoWFactory for CategoryReadUoWFactory {
    type Entity = Category;
    fn create(&self) -> Box<dyn use_cases::ReadUoW<Entity = Category>> {
        Box::new(CategoryReadUoW::new(&self.context))
    }
}

impl use_cases::ReadRelUoWFactory<CategoryRelationshipField> for CategoryReadUoWFactory {
    fn create(&self) -> Box<dyn use_cases::ReadRelUoW<CategoryRelationshipField>> {
        Box::new(CategoryReadUoW::new(&self.context))
    }
}

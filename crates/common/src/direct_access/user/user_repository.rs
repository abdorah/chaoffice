...
pub enum UserRelationshipField {
    Person,
    Session,
}

impl Display for UserRelationshipField {
...
pub struct UserRepository<'a> {
    redb_table: Box<dyn UserTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> UserRepository<'a> {
    pub fn new(redb_table: Box<dyn UserTable + 'a>, transaction: &'a Transaction) -> Self {
        UserRepository {
            redb_table,
            transaction,
        }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::User(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<User>, RepositoryError> {
        self.redb_table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<User>>, RepositoryError> {
        self.redb_table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<User>, RepositoryError> {
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::User(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let session = entity.session.clone();

        // remove all strong relationships, initiating a cascade remove

        if let Some(session_id) = session {
            repository_factory::write::create_session_repository(self.transaction)?
                .remove(event_buffer, &session_id)?;
        }
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Users, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::User(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let session_ids: Vec<EntityId> = entities
            .iter()
            .filter_map(|entity| entity.as_ref().and_then(|entity| entity.session.clone()))
            .collect();

        // remove all strong relationships, initiating a cascade remove

        repository_factory::write::create_session_repository(self.transaction)?
            .remove_multi(event_buffer, &session_ids)?;
        // Before removal, find which owner(s) reference these entities
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_root_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Users, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &RootRelationshipField::Users)
    }
    ...
    pub fn set_relationships_in_owner(
        ...
        repo.set_relationship(event_buffer, owner_id, &RootRelationshipField::Users, ids)
    }
    ...
    pub fn snapshot(&self, ids: &[EntityId]) -> Result<EntityTreeSnapshot, RepositoryError> {
        ...
        {
            // Extract child IDs from the forward junction snapshot for session
            let junction_name = "session_from_user_session_junction";
    ...
    pub fn restore(
        ...
        for child_snap in &snap.children {
            if child_snap.table_data.entity_rows.table_name == "session" {
                repository_factory::write::create_session_repository(self.transaction)?
...
pub struct UserRepositoryRO<'a> {
    redb_table: Box<dyn UserTableRO + 'a>,
}
impl<'a> UserRepositoryRO<'a> {
    pub fn new(redb_table: Box<dyn UserTableRO + 'a>) -> Self {
        UserRepositoryRO { redb_table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<User>, RepositoryError> {
        self.redb_table.get(id)
    }

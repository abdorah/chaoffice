...
pub enum ProductRelationshipField {
    Category,
    Location,
    Supplier,
}

impl Display for ProductRelationshipField {
...
pub struct ProductRepository<'a> {
    redb_table: Box<dyn ProductTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> ProductRepository<'a> {
    pub fn new(redb_table: Box<dyn ProductTable + 'a>, transaction: &'a Transaction) -> Self {
        ProductRepository {
            redb_table,
            transaction,
        }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Product(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Product>, RepositoryError> {
        self.redb_table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Product>>, RepositoryError> {
        self.redb_table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Product>, RepositoryError> {
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Product(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Products, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Product(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_root_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Products, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn set_relationship_multi(
        ...
        if !all_right_ids.is_empty() {
            match field {
                ProductRelationshipField::Category => {
                    let child_repo =
                ...
                ProductRelationshipField::Location => {
                    let child_repo =
                ...
                ProductRelationshipField::Supplier => {
                    let child_repo =
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &RootRelationshipField::Products)
    }
...
pub struct ProductRepositoryRO<'a> {
    redb_table: Box<dyn ProductTableRO + 'a>,
}
impl<'a> ProductRepositoryRO<'a> {
    pub fn new(redb_table: Box<dyn ProductTableRO + 'a>) -> Self {
        ProductRepositoryRO { redb_table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Product>, RepositoryError> {
        self.redb_table.get(id)
    }

...
pub enum PersonRelationshipField {
    Contact,
}

impl Display for PersonRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait PersonTable {
...
pub struct PersonRepository<'a> {
    redb_table: Box<dyn PersonTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> PersonRepository<'a> {
    pub fn new(redb_table: Box<dyn PersonTable + 'a>, transaction: &'a Transaction) -> Self {
        PersonRepository {
            redb_table,
            transaction,
        }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Person(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Person>, RepositoryError> {
        self.redb_table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Person>>, RepositoryError> {
        self.redb_table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Person>, RepositoryError> {
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Person(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let contact = entity.contact.clone();

        // remove all strong relationships, initiating a cascade remove

        if let Some(contact_id) = contact {
            repository_factory::write::create_contact_repository(self.transaction)?
                .remove(event_buffer, &contact_id)?;
        }
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Persons, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Person(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let contact_ids: Vec<EntityId> = entities
            .iter()
            .filter_map(|entity| entity.as_ref().and_then(|entity| entity.contact.clone()))
            .collect();

        // remove all strong relationships, initiating a cascade remove

        repository_factory::write::create_contact_repository(self.transaction)?
            .remove_multi(event_buffer, &contact_ids)?;
        // Before removal, find which owner(s) reference these entities
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_root_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Persons, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &RootRelationshipField::Persons)
    }
    ...
    pub fn set_relationships_in_owner(
        ...
        repo.set_relationship(event_buffer, owner_id, &RootRelationshipField::Persons, ids)
    }
    ...
    pub fn snapshot(&self, ids: &[EntityId]) -> Result<EntityTreeSnapshot, RepositoryError> {
        ...
        {
            // Extract child IDs from the forward junction snapshot for contact
            let junction_name = "contact_from_person_contact_junction";
    ...
    pub fn restore(
        ...
        for child_snap in &snap.children {
            if child_snap.table_data.entity_rows.table_name == "contact" {
                repository_factory::write::create_contact_repository(self.transaction)?
...
pub struct PersonRepositoryRO<'a> {
    redb_table: Box<dyn PersonTableRO + 'a>,
}
impl<'a> PersonRepositoryRO<'a> {
    pub fn new(redb_table: Box<dyn PersonTableRO + 'a>) -> Self {
        PersonRepositoryRO { redb_table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Person>, RepositoryError> {
        self.redb_table.get(id)
    }

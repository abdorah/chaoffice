...
pub enum BudgetEntryRelationshipField {
    Deal,
    Product,
    RecordedBy,
}

impl Display for BudgetEntryRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait BudgetEntryTable {
...
pub struct BudgetEntryRepository<'a> {
    redb_table: Box<dyn BudgetEntryTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> BudgetEntryRepository<'a> {
    pub fn new(redb_table: Box<dyn BudgetEntryTable + 'a>, transaction: &'a Transaction) -> Self {
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::BudgetEntry(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<BudgetEntry>, RepositoryError> {
        self.redb_table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<BudgetEntry>>, RepositoryError> {
        self.redb_table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<BudgetEntry>, RepositoryError> {
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::BudgetEntry(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::BudgetEntries, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::BudgetEntry(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_root_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::BudgetEntries, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn set_relationship_multi(
        ...
        if !all_right_ids.is_empty() {
            match field {
                BudgetEntryRelationshipField::Deal => {
                    let child_repo =
                ...
                BudgetEntryRelationshipField::Product => {
                    let child_repo =
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &RootRelationshipField::BudgetEntries)
    }
...
pub struct BudgetEntryRepositoryRO<'a> {
    redb_table: Box<dyn BudgetEntryTableRO + 'a>,
}
impl<'a> BudgetEntryRepositoryRO<'a> {
    pub fn new(redb_table: Box<dyn BudgetEntryTableRO + 'a>) -> Self {
        BudgetEntryRepositoryRO { redb_table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<BudgetEntry>, RepositoryError> {
        self.redb_table.get(id)
    }

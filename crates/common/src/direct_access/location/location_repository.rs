...
pub enum LocationRelationshipField {}

impl Display for LocationRelationshipField {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{:?}", self)
    }
}

pub trait LocationTable {
    ...
    fn update_with_relationships(&mut self, entity: &Location)
...
pub struct LocationRepository<'a> {
    redb_table: Box<dyn LocationTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> LocationRepository<'a> {
    pub fn new(redb_table: Box<dyn LocationTable + 'a>, transaction: &'a Transaction) -> Self {
        LocationRepository {
            redb_table,
            transaction,
        }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Location(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Location>, RepositoryError> {
        self.redb_table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Location>>, RepositoryError> {
        self.redb_table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Location>, RepositoryError> {
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Location(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Locations, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Location(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_root_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Locations, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &RootRelationshipField::Locations)
    }
...
pub struct LocationRepositoryRO<'a> {
    redb_table: Box<dyn LocationTableRO + 'a>,
}
impl<'a> LocationRepositoryRO<'a> {
    pub fn new(redb_table: Box<dyn LocationTableRO + 'a>) -> Self {
        LocationRepositoryRO { redb_table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Location>, RepositoryError> {
        self.redb_table.get(id)
    }

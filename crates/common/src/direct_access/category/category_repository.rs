...
pub enum CategoryRelationshipField {
    ParentCategory,
    Subcategories,
}

impl Display for CategoryRelationshipField {
...
pub trait CategoryTable {
    ...
    fn update_with_relationships(&mut self, entity: &Category)
...
pub struct CategoryRepository<'a> {
    redb_table: Box<dyn CategoryTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> CategoryRepository<'a> {
    pub fn new(redb_table: Box<dyn CategoryTable + 'a>, transaction: &'a Transaction) -> Self {
        CategoryRepository {
            redb_table,
            transaction,
        }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Category(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Category>, RepositoryError> {
        self.redb_table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Category>>, RepositoryError> {
        self.redb_table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Category>, RepositoryError> {
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Category(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Categories, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Category(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_root_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Categories, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &RootRelationshipField::Categories)
    }
...
pub struct CategoryRepositoryRO<'a> {
    redb_table: Box<dyn CategoryTableRO + 'a>,
}
impl<'a> CategoryRepositoryRO<'a> {
    pub fn new(redb_table: Box<dyn CategoryTableRO + 'a>) -> Self {
        CategoryRepositoryRO { redb_table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Category>, RepositoryError> {
        self.redb_table.get(id)
    }

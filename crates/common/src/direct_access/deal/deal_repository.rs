...
pub enum DealRelationshipField {
    Manager,
    Product,
    Supplier,
}

impl Display for DealRelationshipField {
...
pub struct DealRepository<'a> {
    redb_table: Box<dyn DealTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> DealRepository<'a> {
    pub fn new(redb_table: Box<dyn DealTable + 'a>, transaction: &'a Transaction) -> Self {
        DealRepository {
            redb_table,
            transaction,
        }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Deal(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Deal>, RepositoryError> {
        self.redb_table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Deal>>, RepositoryError> {
        self.redb_table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Deal>, RepositoryError> {
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Deal(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            ...
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Deals, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Deal(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_root_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&RootRelationshipField::Deals, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn set_relationship_multi(
        ...
        if !all_right_ids.is_empty() {
            match field {
                ...
                DealRelationshipField::Supplier => {
                    let child_repo =
                        repository_factory::write::create_person_repository(self.transaction)?;
                    let found = child_repo.get_multi(&all_right_ids)?;
                    let missing: Vec<_> = all_right_ids
    ...
    pub fn get_relationships_from_owner(
        ...
        repo.get_relationship(owner_id, &RootRelationshipField::Deals)
    }
    ...
    pub fn set_relationships_in_owner(
        ...
        repo.set_relationship(event_buffer, owner_id, &RootRelationshipField::Deals, ids)
    }
...
pub struct DealRepositoryRO<'a> {
    redb_table: Box<dyn DealTableRO + 'a>,
}
impl<'a> DealRepositoryRO<'a> {
    pub fn new(redb_table: Box<dyn DealTableRO + 'a>) -> Self {
        DealRepositoryRO { redb_table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Deal>, RepositoryError> {
        self.redb_table.get(id)
    }

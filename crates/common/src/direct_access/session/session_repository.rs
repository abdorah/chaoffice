...
use crate::direct_access::user::UserRelationshipField;
use crate::error::RepositoryError;
use serde::{Deserialize, Serialize};

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum SessionRelationshipField {}

impl Display for SessionRelationshipField {
...
pub struct SessionRepository<'a> {
    redb_table: Box<dyn SessionTable + 'a>,
    transaction: &'a Transaction,
}

impl<'a> SessionRepository<'a> {
    pub fn new(redb_table: Box<dyn SessionTable + 'a>, transaction: &'a Transaction) -> Self {
        SessionRepository {
            redb_table,
            transaction,
        }
    ...
    pub fn create_orphan(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Session(EntityEvent::Created)),
            ids: vec![new.id],
            data: None,
        });
        Ok(new)
    ...
    pub fn get(&self, id: &EntityId) -> Result<Option<Session>, RepositoryError> {
        self.redb_table.get(id)
    }
    pub fn get_multi(&self, ids: &[EntityId]) -> Result<Vec<Option<Session>>, RepositoryError> {
        self.redb_table.get_multi(ids)
    }
    pub fn get_all(&self) -> Result<Vec<Session>, RepositoryError> {
    ...
    pub fn update(
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Session(EntityEvent::Updated)),
            ids: vec![updated.id],
            data: None,
        });
        Ok(updated)
    ...
    pub fn remove(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_user_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&UserRelationshipField::Session, &[*id])?
                .into_iter()
        ...
        event_buffer.push(Event {
            origin: Origin::DirectAccess(DirectAccessEntity::Session(EntityEvent::Removed)),
            ids: vec![*id],
            data: None,
        });
    ...
    pub fn remove_multi(
        ...
        let affected_owner_ids: Vec<EntityId> = {
            let owner_repo = repository_factory::write::create_user_repository(self.transaction)?;
            owner_repo
                .get_relationships_from_right_ids(&UserRelationshipField::Session, ids)?
                .into_iter()
                .map(|(owner_id, _)| owner_id)
                .collect()
        };
    ...
    pub fn get_relationships_from_owner(
        ...
        let repo = repository_factory::write::create_user_repository(self.transaction)?;
        repo.get_relationship(owner_id, &UserRelationshipField::Session)
    }
    ...
    pub fn set_relationships_in_owner(
        ...
        let mut repo = repository_factory::write::create_user_repository(self.transaction)?;
        repo.set_relationship(event_buffer, owner_id, &UserRelationshipField::Session, ids)
    }
...
pub struct SessionRepositoryRO<'a> {
    redb_table: Box<dyn SessionTableRO + 'a>,
}
impl<'a> SessionRepositoryRO<'a> {
    pub fn new(redb_table: Box<dyn SessionTableRO + 'a>) -> Self {
        SessionRepositoryRO { redb_table }
    }
    pub fn get(&self, id: &EntityId) -> Result<Option<Session>, RepositoryError> {
        self.redb_table.get(id)
    }

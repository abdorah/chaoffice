...
use crate::use_cases::login_uc::{LoginUnitOfWorkFactoryTrait, LoginUnitOfWorkTrait};
...
// Unit of work for Login

pub struct LoginUnitOfWork {
    context: DbContext,
...
impl CommandUnitOfWork for LoginUnitOfWork {
...
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/login_uc.rs
//
// Exactly the same macros must be set in the use case uow trait file in ../use_cases/login_uc.rs
...
impl LoginUnitOfWorkTrait for LoginUnitOfWork {}

pub struct LoginUnitOfWorkFactory {
    context: DbContext,
    event_hub: Arc<EventHub>,
}

impl LoginUnitOfWorkFactory {
    pub fn new(db_context: &DbContext, event_hub: &Arc<EventHub>) -> Self {
        LoginUnitOfWorkFactory {
            context: db_context.clone(),
            event_hub: event_hub.clone(),
        }
    }
...
impl LoginUnitOfWorkFactoryTrait for LoginUnitOfWorkFactory {
    fn create(&self) -> Box<dyn LoginUnitOfWorkTrait> {
        Box::new(LoginUnitOfWork::new(&self.context, &self.event_hub))
    }
}

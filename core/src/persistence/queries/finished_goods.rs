...
pub async fn list_all(pool: &SqlitePool) -> AppResult<Vec<FinishedGoodRow>> {
    let rows = sqlx::query_as::<_, FinishedGoodRow>(
        "SELECT id, name, current_quantity, unit_price, last_updated, sync_status, updated_at FROM finished_goods ORDER BY name ASC",
    )
    .fetch_all(pool)
    .await?;
...
/// Deduct `amount` from a finished good's quantity.
///

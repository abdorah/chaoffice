...
pub fn export_to_excel(invoice: &Invoice) -> AppResult<Vec<u8>> {
    ...
    for (i, item) in invoice.line_items.iter().enumerate() {
        let subtotal = item.unit_price.to_f64() * item.quantity as f64;
        ...
        sheet.write_number_with_format(row, 3, item.unit_price.to_f64(), &money_fmt).map_err(xlsx_err)?;
    ...
    sheet.write_number_with_format(row, 4, invoice.total_amount.to_f64(), &bold_money).map_err(xlsx_err)?;
    row += 1;
    sheet.write_string(row, 3, "Amount Paid:").map_err(xlsx_err)?;
    sheet.write_number_with_format(row, 4, invoice.amount_paid.to_f64(), &money_fmt).map_err(xlsx_err)?;
    row += 1;
    sheet.write_string_with_format(row, 3, "Remaining:", &bold).map_err(xlsx_err)?;
    sheet.write_number_with_format(row, 4, invoice.remaining_balance.to_f64(), &bold_money).map_err(xlsx_err)?;
...
mod tests {
    ...
    fn export_to_excel_empty_line_items() {
        ...
        invoice.total_amount = Money::ZERO;
        invoice.amount_paid = Money::ZERO;
        invoice.remaining_balance = Money::ZERO;

...
async fn seed_wallet(pool: &sqlx::SqlitePool, id: &str, name: &str, balance_cents: i64) {
    let now = Utc::now().to_rfc3339();
    wallet_queries::insert_wallet(pool, id, name, "Cash", balance_cents, &now)
        .await
...
proptest! {
    ...
    fn prop25_expense_debits_wallet_and_contains_all_fields(
        ...
        balance_cents in 1000i64..1_000_000i64,
        ...
        rt.block_on(async {
            ...
            let initial_balance = Money(balance_cents);
            let amount_cents = (balance_cents * amount_pct as i64 / 100).max(1);
            let amount = Money(amount_cents);

            seed_wallet(&pool, &wallet_id.to_string(), "Test Wallet", initial_balance.0).await;
            ...
            prop_assert_eq!(
                wallet_row.current_balance, expected_balance.0,
                "Wallet balance should be {} but was {} (initial={}, amount={})",
                expected_balance, wallet_row.current_balance, initial_balance, amount
            );
...
proptest! {
    ...
    fn prop31_expense_report_grouping_and_totals(
        ...
        rt.block_on(async {
            ...
            let total_needed = total_cents as i64 + 100;
            seed_wallet(&pool, &wallet_id.to_string(), "Big Wallet", total_needed).await;
            seed_user(&pool, &user_id.to_string()).await;

            // Track expected amounts per category
            let mut expected_purchase_total = Money::ZERO;
            let mut expected_operating_total = Money::ZERO;
            ...
            for (i, (cents, cat_flag)) in expense_specs.iter().enumerate() {
                ...
                match &category {
                    ExpenseCategory::Purchase => {
                        expected_purchase_total = expected_purchase_total + amount;
                        expected_purchase_count += 1;
                    }
                    ExpenseCategory::OperatingCost => {
                        expected_operating_total = expected_operating_total + amount;
            ...
            let actual_purchase_total: Money = purchase_expenses
                .map_or(Money::ZERO, |v| v.iter().map(|e| e.amount).sum());
            let actual_operating_total: Money = operating_expenses
                .map_or(Money::ZERO, |v| v.iter().map(|e| e.amount).sum());

            prop_assert_eq!(
                actual_purchase_total, expected_purchase_total,
                "Purchase subtotal: expected {} got {}",
                expected_purchase_total, actual_purchase_total
            );
            prop_assert_eq!(
                actual_operating_total, expected_operating_total,
